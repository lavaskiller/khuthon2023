...
def app():
    ...
    if not st.session_state.signout:
        ...
        deta.Base("users_db").get(st.session_state.useremail)["uses"]
        <= deta.Base("users_db").get(st.session_state.useremail)["limit"]
    ):
        # Display chat messages from history on app rerun
        for message in [
            {"content": item["content"], "role": item["role"]}
            for item in deta.Base("chat_log")
            .fetch({"user": st.session_state.useremail})
            .items
        ]:
