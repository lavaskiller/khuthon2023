        ...
        if prompt := st.chat_input("Hello! Who are you?"):
            ...
            if user.get(st.session_state.useremail)["state"] == "begin":
                gpt_prompt = [
                    {
                        "role": "system",
                        "content": "Analyze the user's question to identify the core topic, which we will define as main_key. Think of the correct answer to the question and define it as anw. [You should not directly reveal anw.] Remember, you are an educator, not just a provider of answers. Your role is to guide the student to discover anw on their own. To achieve this, you can explain the main concepts which must be need for understanding anw but not realated to anw or pose guiding questions that lead to anw. Your response to the student will be defined as resp in Korean. Output main_key, anw, resp in JSON format. Before finalizing the output, ensure it meets the following conditions: (1) Yun MUST NOT reveal anw in resp!!!!, (2) the output must be in JSON format like {} so that I can straightly use json.loads, and (3) it must consist of main_key, anw, and resp. (4) resp must be written in Korean.",
                    }
                ]
            ...
            with st.spinner("답변이 생성되는 중입니다..."):
                try:
                    response = openai.ChatCompletion.create(
                        model="gpt-4-1106-preview", messages=gpt_prompt, stream=False
                    )
